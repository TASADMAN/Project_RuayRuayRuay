...
<body>
	...
	<script type="text/javascript">
		...
		function drawBasic() {
			...
			var data = google.visualization.arrayToDataTable([
					...
					String sqls = "SELECT SUM(receipt_detalis.qua) AS total FROM receipt_detalis WHERE receipt_detalis.good_id ="
					+ rec.getInt("id");
					re = bs.executeQuery(sqls);
					re.next();%>
					[ '<%=rec.getString("name")%>', <%=re.getInt("total")%>, '#E6A07B'],
	<%num = num + 1;
}%>
		]);
	<%try {
	...
	if (Connect != null) {
		...
		String sql = "SELECT * FROM receipt_detalis INNER JOIN GOODS ON receipt_detalis.good_id = goods.id ORDER BY receipt_detalis.id";
	...
	<div class="container-sm " align="center" style="margin-top: 5rem;">
		<div align="center">
			<h2 style="margin-left: 5rem">Table Product Sales</h2>
		</div>
		<div align="center">
			<table class="table text-center" style="width: 70%">
				...
				<tbody>
					...
					<tr>
						...
						<td><%=res.getInt("qua")%></td>
						<td><%=res.getDouble("amount")%></td>
					</tr>
					<%
					nums = nums + 1;
					}
